...
import { MapModal } from '../Helper/Modals/DisasterRiskModal'
import { EventDetailModal } from '../Helper/Modals/DisasterRiskModal';
...
import { predictedData } from '../../store/slices/disasterSlice';
...
const EventTable = () => {
  ...
  useEffect(() => {
    const handleDisaster = async () => {
      const result = await dispatch(predictedData({ req: 'allData' }))
      setRequestData(result?.data?.data)
      setActive("disaster")
      const forecast = await getForecast()
      setOverviewData([{ id: 0, title: "Predicted Disasters", count: result?.data?.data.length },

      { id: 1, title: "Submitted Requests", count: forecast.length }
      ])
    }
    handleDisaster();
  }, [])
  ...
  const handlePersonal = async () => {
    ...
    const result = data.filter((data) => data.userId === userId)
    setRequestData(result)
    setActive("personal")
  }
  ...
  return (
    <>
            ...
            <main className='relative  h-full w-full mx-auto max-w-11xl bg-teal-50 flex items-start justify-between ' ref={sectionRef}>
              ...
              <div className='relative h-full max-lg:w-full lg:w-[70%] flex flex-col max-sm:text-sm sm:text-sm lg:text-lg  font-medium '>
                ...
                <div className='w-[90%] mx-auto' ref={dropdownRef}>
                  ...
                  <div className='w-full flex items-center justify-between gap-2 max-md:flex-wrap'>
                    <div className=' w-full h-full flex  items-center justify-center gap-5'>
                      <Button size='sm' variant='flat' color={active === "disaster" ? "secondary" : "success"} radius='sm' onClick={handleDisaster} className='mt-1 xl:text-2xl font-serif font-medium'>Disasters</Button>
                      <Button size='sm' variant='flat' color={active === "report" ? "secondary" : "success"} radius='sm' onClick={handleReport} className='mt-1 xl:text-2xl font-serif font-medium'>Submitted </Button>
                    </div>
                    <div className='sm:hidden w-full h-full flex  items-center justify-end max-md:justify-center gap-5'>
                      <Button size='sm' variant='flat' color={active === "personal" ? "secondary" : "success"} radius='sm' onClick={handlePersonal} className='mt-1 xl:text-2xl font-serif font-medium'>Your Req</Button>
                      <Button size='sm' variant='flat' color={active === "other" ? "secondary" : "success"} radius='sm' onClick={handleOther} className='mt-1 xl:text-2xl font-serif font-medium'>Other Req</Button>
                    </div>
                  ...
                  {filtered?.map((data, index) => (
                    <div key={index}>
                      <div onClick={() => handleDetail(data)} className=' cursor-pointer relative h-full w-full border-5 p-1 my-2 rounded-md '>
                        <div className='absolute inset-0 '>
                          <div className='absolute h-[50%] max-md:h-[45%] max-sm:h-[29%]  md:w-[37%] max-md:w-[48%] max-sm:w-[90%] blur-[2px] bg-white rounded-tl-xl  bottom-0 right-0 '></div>
                        ...
                        <div className='relative h-full w-full flex flex-col'>
                          <div key={''} className='relative h-full w-full max-sm:flex-col flex max-sm:items-center items-start justify-evenly max-sm:flex-wrap gap-2'>
                            ...
                            <img src={data.ipfsUrl || "https://via.placeholder.com/150"} alt="Disaster img" className='h-[120px] w-[20%] max-sm:h-[110px] max-sm:w-[50%] max-sm:mx-auto rounded-md max-sm:order-2' />
              ...
              <div className='max-sm:hidden sticky top-0 h-screen flex flex-col items-center justify-evenly max-2xl:w-[28%] w-[28%]'>
                <div className='absolute inset-0 -z-10000 bg-transparent blur-[4px]'>
                  <div className='absolute h-[10%] max-md:h-[10%]  md:w-[95%] max-md:w-[90%] max-sm:w-[68%]  blur-[2px] bg-white  bottom-0 right-0 '></div>
                  <Spline
                    //scene="https://prod.spline.design/19vkhsMOvXaLN8X4/scene.splinecode"
                    //scene="https://prod.spline.design/oWIxdLDOFI8Z6ATf/scene.splinecode"
                    scene="https://prod.spline.design/JL5jsKmpBhOh7TAx/scene.splinecode"
      ...
      <EventDetailModal isOpen={openDetailModal} onOpenChange={changeDetailModal} detail={detailData} />
    </>
  )
}
